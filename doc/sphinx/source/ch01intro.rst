...
elementary algebraic operation, the inner product. The first objective is to
compute the inner product as accurate as possible without violating the second
objective to do it as efficient as possible for middle and large dimension
vector lengths. Hereby middle dimension describes vectors with about
:math:`10^{3}` elements and accordingly large dimension vectors with more than
:math:`10^{6}` elements. [#f1]_
...
instruction is one of the main reasons for writing this thesis, as it becomes
available as hardware realization for upcoming generations of consumer
processors. The :term:`FMA` instruction itself and the usage from the
programming languages C and C++ are analyzed comprehensively in Chapter
:ref:`ch-fma`.
...
information`. Finally, the results of this thesis are concluded in Chapter
:ref:`ch-conclusion` with a perspective for the future.
